...
class CreateTeamleadsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('teamleads', function (Blueprint $table) {
            ...
            $table->string('teamlead_official_id',150)->unique();
            $table->string('first_name');
            $table->string('last_name');
            $table->string('email',150)->unique();
            $table->string('phone_number');
            $table->integer('teamlead_status')->default('1');
            $table->integer('rating')->default('300');
            $table->string('gender');
            $table->date('date_of_birth');
            $table->bigInteger('teamlead_salary');
            $table->string('teamlead_account_number');
            $table->string('qualification');
            $table->string('user_name',150)->unique();
            $table->string('password');
            $table->text('address');
            $table->text('teamlead_image');
            $table->timestamps();
        });
    }
